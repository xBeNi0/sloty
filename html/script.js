...
function insertCoin(amount) {
  ...
  $.post("http://smx_slots/updateChips", JSON.stringify({
    coinAmount: coins
  }));
}
...
function endWithWin(x, sound) {
  ...
  $('.betUp').empty().append("KIER");
  $('.AllIn').empty().append("PIK");
  $('.go').empty().append("");
...
function voteColor(x, color) {
  ...
  for(var cont = colorHistory.length; cont >= colorHistory.length-8; cont--) {
    ...
    $('#h' + pls).empty();
    if(imgColor !== "none") {
      $('#h' + pls).append("<img src='img/" + imgColor + ".png' width=60px height=60px/>");
      pls++;
    }
  }
...
function togglePacanele(start, banuti) {
  if(start == true) {
    ...
    $.post("http://smx_slots/exitWith", JSON.stringify({
